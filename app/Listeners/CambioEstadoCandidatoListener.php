<?php

namespace App\Listeners;

use App\Events\ProcesoReclutamientoCandidato;
use Illuminate\Queue\InteractsWithQueue;
use Illuminate\Contracts\Queue\ShouldQueue;
use App\Models\ReqCandidato;

class CambioEstadoCandidatoListener {

    /**
     * Create the event listener.
     *
     * @return void
     */
    public function __construct() {
        //
    }

    /**
     * Handle the event.
     *
     * @param  ProcesoReclutamientoCandidato  $event
     * @return void
     */
    public function handle(ProcesoReclutamientoCandidato $event) {
        //ACTUALIZO REGISTRO A ESTADO QUITADO

        $reqCandidato = ReqCandidato::find($event->candidato->id);
        $reqCandidato->fill(["estado_candidato" => $event->estado]);
        $reqCandidato->save();
    }

}
