...
class SessionTimeoutReclutamiento {
    protected $session;
    ...
    public function handle($request, Closure $next)
    {
        ...
        elseif(time() - $this->session->get('lastActivityTime') > $this->timeout){
            ...
            Sentinel::logout();
            return redirect()->route("reclutamiento_externo.login")->with(['mensaje_error' => 'El usuario no ha tenido actividad en '.$this->timeout/60 .' minutos.']);
        }
        $this->session->put('lastActivityTime',time());
