    ...
    <div>
        ...
        <table class="table">
            ...
            <tbody id="table_adicionales_cliente">
                @foreach($adicionales_cliente as $adicional)
                    <?php
                        $informacion_adicional = $adicional->adicional_cargo($registro->id);
                    ?>
                    ...
                    <tr class="item_adicional">
                        <td width="8%">
                            <input 
                                type="checkbox" 
                                class="contratacion-clausulas" 
                                name="clausulas[]" 
                                id="check_adicionales" 
                                value="{{ $adicional->id }}"

                                {{ ($registro->hasDocumentoAdicional($registro->id, $adicional->id)) ? 'checked' : '' }}
                            >
                        </td>
                        ...
                        @if(preg_match('/{valor_variable}/', $adicional->contenido_clausula))
                            <td width="40%">
                                <input 
                                    ...
                                    @if ($informacion_adicional->active)
                                        value="{{ $adicional->variableCargo($registro->id)->valor }}"
                                    @else
                                        disabled
                                    @endif
