<div class="modal-header">
 <button aria-label="Close" class="close" data-dismiss="modal" type="button">
   <span aria-hidden="true"> × </span> </button>
    <h4 class="modal-title" id="myModalLabel"> Cargar Certificado Estudio </h4>
</div>
<div class="modal-body">
   {!! Form::open(["class"=>"form-horizontal", "role"=>"form","files"=>true,"id"=>"form_carga_certificado_estudio"]) !!}
    <input type="hidden" name="estudio_id" value="{{$estudio_id}}">
    <div class="row">
      <div class="col-md-12">
...
<style>
    .swal-overlay{ z-index: 2000; }
</style>
<script type="text/javascript">
    $(document).ready(function(){
        //Guardar archivo HV
        $("#guardar_certificado").on("click", function () {
            var formData = new FormData(document.getElementById("form_carga_certificado_estudio"));

            $.ajax({
                type: "POST",
                data: formData,
                url: "{{ route('guardar_certificado_estudio') }}",
                cache: false,
                contentType: false,
                processData: false,
